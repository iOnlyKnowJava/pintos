...
void child_info_init (struct child_info *info)
{
  ...
  sema_init (&info->control_sema, 1);
  info->other_exited = false;
}
                     ...
                     void *aux)
{
  ...
  list_push_back (&thread_current ()->children_infos, &info->elem);
...
void thread_exit (void)
{
  ...
  if (thread_current ()->info_to_update != NULL)
    {
      sema_down (&thread_current ()->info_to_update->control_sema);
      if (thread_current ()->info_to_update->other_exited)
        {
          free (thread_current ()->info_to_update);
        }
      else
        {
          thread_current ()->info_to_update->other_exited = true;
          sema_up (&thread_current ()->info_to_update->control_sema);
        }
    }

  // Free all child_infos where child has exited already
  while (!list_empty (&thread_current ()->children_infos))
    {
      struct child_info *info =
          list_entry (list_pop_front (&thread_current ()->children_infos),
                      struct child_info, elem);
      sema_down (&info->control_sema);
      if (info->other_exited)
        {
          free (info);
        }
      else
        {
          info->other_exited = true;
          sema_up (&info->control_sema);
        }
    }
...
{
  struct thread *curr = thread_current ();
  return max (curr->priority, curr->donated_priority);
}

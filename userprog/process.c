...
tid_t process_execute (const char *file_name)
{
  ...
  char *save_ptr = NULL;
  char namecopy[15]; // Copy of name that can be modified/parsed
  strlcpy (namecopy, file_name, sizeof (namecopy));

  // Get actual name by parsing
  const char *name = strtok_r (namecopy, " \t\n", &save_ptr);
...
static void start_process (void *file_name_)
{
  ...
  // Let parent know if exec successful
...
{
  struct list_elem *e;
  /* Find child to wait on in children_infos */
  if (!list_empty (&thread_current ()->children_infos))
    {
      for (e = list_begin (&thread_current ()->children_infos);
           e != list_end (&thread_current ()->children_infos);
           e = list_next (e))
        {
          struct child_info *f = list_entry (e, struct child_info, elem);
          if (f->tid == child_tid)
            {
              list_remove (e); // prevent waiting twice
              // wait for child to exit
              sema_down (&f->wait_sema);
              int ret_val = f->exit_status;

              // Free if child finished with exiting
              sema_down (&f->control_sema);
              if (f->other_exited)
                {
                  free (f);
                }
...
bool load (const char *file_name, void (**eip) (void), void **esp)
{
  ...
  /* Set up stack. */
  if (!setup_stack (esp, arguments, save_ptr))
    goto done;
